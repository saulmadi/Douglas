...
namespace Douglas.Domain.Specs
{
        ...
        Establish context = () =>
            {
                ...
                _command = new CreateEmailLoginUser(
                    "email", new EncryptedPassword("password"), "name", "password", _userAbilities);

                _userCreated =
                    Builder<UserEmailLogin>.CreateNew()
                ...
                Mock.Get(_readOnlyRepository)
                    .Setup(repository => repository.GetById<UserAbility>(_userAbility.Id))
                    .Returns(_userAbility);
                ...
                _expectedEvent = new UserEmailCreated(
                    _userCreated.Id, _command.Email, _command.Name, _command.PhoneNumber);
                _handler.NotifyObservers += x => _eventRaised = x;
            };

        Because of = () => _handler.Handle(Mock.Of<IUserSession>(), _command);
        ...
        It should_create_the_new_user =
            ...
            Mock.Get(_writeableRepository)
                .Verify(
                    x =>
                    x.Create(
                        Moq.It.Is<UserEmailLogin>(
                            u =>
                            u.Name == _command.Name && u.Email == _command.Email
                            && u.EncryptedPassword == _command.EncryptedPassword.Password
                            && u.PhoneNumber == _command.PhoneNumber && u.UserAbilities.Contains(_userAbility))));

        It should_throw_the_expected_event = () => _eventRaised.ShouldBeLike(_expectedEvent);
    }
}